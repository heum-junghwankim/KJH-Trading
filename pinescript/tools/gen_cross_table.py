...
■ 알고리즘 수학자 리뷰 반영 (A군: 구조 개선, 신규 데이터 불필요) — 2026 개정
  ...
  - A-2 모순조합 자동 무효화 + 플래그: 방향=롱 & 구름=아래(또는 반대)면 ichi_net=0 → ichi_sign=0으로
        자동 상쇄. 추가로 방향·구름이 상충하는 행에는 '이치모쿠 내부충돌' 플래그를 종합해석에 표기.
...
def main():
    ...
    conflict = sum(1 for r in rows if "이치모쿠 내부충돌" in r["verdict"])
    print(f"이치모쿠 내부충돌 행: {conflict}")
