...
| 행 | 내용 |
| --- | --- |
| ADX | 값 + `충분`(≥ ADX Minimum) / `약함`. 셀 색은 **DI 방향**(+DI 우위=초록, -DI 우위=빨강), **충분=선명한 색 / 약함=채도 낮은 둔탁한 색**(포지션 약화와 동일 톤) |
| DI 방향 | `+DI 우위(상승)` / `-DI 우위(하락)` |
| 추세 과열 | `되돌림 주의(반등)` / `되돌림 주의(하락)` / `-`. 추세가 통계적 극단까지 과도하게 진행돼 되돌림 위험이 큰 구간 |
| 포지션 | `롱 유리(강화/약화)` / `숏 유리(강화/약화)` / `중립(휩쏘)` |

`포지션` 판정 — 방향(DI)·추세(ADX)에 **OBV 방향 에너지의 강화/약화**를 더합니다:
- `롱 유리`: ADX 충분 + `+DI 우위` / `숏 유리`: ADX 충분 + `-DI 우위`
- **강화 vs 약화**: OBV 방향 에너지 `|+DI − −DI|`가 최근 평균(`OBV 에너지 평균 길이`)보다 **크고 증가** 중이면 `강화`(채도 높은 선명한 색), 평균 이하이거나 **최근 줄어드는 중**이면 `약화`(채도 낮은 탁한 색).
  - 방향·ADX는 그대로여도 매수/매도 에너지가 소진되는 구간을 `약화`로 잡아, 추세 끝물에서 그대로 따라붙는 것을 줄입니다.
...
| `OBV 에너지 평균 길이` | 강화/약화 판정의 기준 평균 구간. 짧으면 민감(약화 자주), 길면 둔감 |
