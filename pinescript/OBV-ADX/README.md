...
| 요소 | 현재 표시 방식 |
| --- | --- |
| `+DI / -DI / ADX` 선 | 옵션을 켜면 OBV 기반 방향성과 강도를 선으로 표시합니다. 세 선의 색은 설정에서 각각 지정할 수 있습니다(기본값 `+DI = 파랑`, `-DI = 주황`, `ADX = 빨강`). |
...
| 극단 표시 | 낙폭과대 / 초과매수는 배경색과 우측 상단 상태판 `ADX` 행의 `(과열)` 접미·셀 색으로 표시합니다(별도 삼각형 마커는 없음). |
...
| ADX | 값 + `충분`(≥ ADX Minimum) / `약함`. **과열이면 끝에 `(과열)` 접미**(예: `충분 (과열)`). 셀 색은 기본적으로 **DI 방향**(+DI 우위=초록, -DI 우위=빨강)·**충분=선명한 색 / 약함=채도 낮은 둔탁한 색**(포지션 약화와 동일 톤)이나, **과열 구간에서는 방향 대신 과열측 색으로 덮어씀** — 롱측 과열(되돌림 주의(반등), 낙폭과대)=**둔한 녹색 `#6E8F72`**, 숏측 과열(되돌림 주의(하락), 초과매수)=**둔한 빨강 `#A57A7A`** |
| DI 방향 | `+DI 우위(상승)` / `-DI 우위(하락)` |

> 이전 버전의 별도 `추세 과열` 행은 삭제하고, 그 정보를 **ADX 행에 병합**했습니다. 과열 상태 자체(낙폭과대 / 초과매수)는 배경색과 `(과열)` 접미·ADX 셀 색으로 함께 드러납니다.
...
| `+DI / -DI / ADX Line Color` | 선 모드에서 세 선의 색(및 투명도)을 취향대로 바꾸고 싶을 때 |
