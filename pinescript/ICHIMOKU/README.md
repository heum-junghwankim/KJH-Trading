...
기본 설정은 `default_qty_type=strategy.percent_of_equity`, `default_qty_value=100`, `pyramiding 0`, `process_orders_on_close=true`, `use_bar_magnifier=true`, `margin_long=100`, `margin_short=100`, `initial_capital=10000`입니다. 레버리지는 없습니다(`margin 100`). 다만 마진콜 방지 로직이 없으므로, `숏` 모드에서 가격이 크게 급등하면 강제청산이 발생할 수 있습니다. 수수료와 슬리피지는 코드에서 가정하지 않았습니다. 실제 거래소 기준으로 보려면 TradingView 전략 속성에서 수수료와 슬리피지를 직접 넣어 주세요.
