...
| 다이버전스 강조색 | `가격 MA 기울기`와 `히스토그램 방향`이 반대로 움직일 때만 히스토그램 색을 바꿉니다. |
...
즉 이 지표에서 말하는 다이버전스는 `클래식 피벗 다이버전스`가 아니라, `가격 이평 기울기와 히스토그램 방향의 어긋남`을 강조하는 방식입니다.
...
| 초록 계열 히스토그램 강조 | 가격 이평은 눌리는데 히스토그램은 먼저 살아나는 상태입니다. |
| 빨강 계열 히스토그램 강조 | 가격 이평은 반등하는데 히스토그램은 먼저 식는 상태입니다. |
...
1. [`비정상 가격 추적 (캔들)`](../비정상%20가격%20추적%20(캔들)/README.md)에서 자리와 기준 봉/후보 흐름을 먼저 확인합니다.
