...
| 리포트 | 롱/숏 이벤트 시그널을 나열하고, 반대 방향 시그널이 나오기 전까지의 누적 상태를 `ON(개수)`로 보여줍니다. 또한 최초 시그널 종가와 현재 봉 시가 사이의 가격 변화도 함께 보여줍니다. | `리포트 표시`, `리포트 표시 위치`, `리포트 글자 크기` |
...
6. 마지막으로 `리포트`에서 롱/숏 이벤트 시그널이 각각 반대 방향 시그널 전까지 얼마나 누적됐는지 `ON(개수)`로 확인합니다.

## 리포트 읽는 법

리포트는 `반대 신호 전 누적 | 롱 | 숏` 3열 구조입니다. 좌상단에는 현재 집계 기준이 `반대 신호 전 누적`으로 표시됩니다.

핵심은 지표가 `롱 우세 / 숏 우세`를 대신 판단하는 것이 아니라, 반대 방향 시그널이 나오기 전까지 각 이벤트가 몇 번 누적됐는지를 바로 확인하는 데 있습니다.

| 항목 | 의미 |
| --- | --- |
| 비정상 시작 | 반대 방향 시그널이 나오기 전까지 비정상 과매수/과매도 시작 이벤트가 몇 번 누적됐는지 보여줍니다. |
| 비정상 유지 | 반대 방향 시그널이 나오기 전까지 비정상 과매수/과매도 캔들이 몇 번 누적됐는지 보여줍니다. |
| 다이버전스 | 반대 방향 시그널이 나오기 전까지 강세/약세 다이버전스가 몇 번 누적됐는지 보여줍니다. |
| ATR 배경 | 반대 방향 시그널이 나오기 전까지 ATR 배경 구간이 몇 번 누적됐는지 보여줍니다. |
| ATR 강화 | 반대 방향 시그널이 나오기 전까지 강한 ATR 배경이 몇 번 누적됐는지 보여줍니다. |
| 진입 후보 | 반대 방향 시그널이 나오기 전까지 롱/숏 진입 후보 이벤트가 몇 번 누적됐는지 보여줍니다. |
| 스윕 확인 | 반대 방향 시그널이 나오기 전까지 스윕 뒤 확인 다이아몬드 이벤트가 몇 번 누적됐는지 보여줍니다. |
| 가격 변화 | 반대 방향 시그널이 나오기 전까지 유지된 현재 누적 구간의 `최초 시그널 종가`와 `현재 봉 시가` 사이 가격 변화를 보여줍니다. 첫 줄에는 변동률, 둘째 줄에는 `yy-MM-dd HH:mm` 형식의 최초 시그널 종가 시간을 표시합니다. |

표 안의 값은 단순합니다.

- `ON(3)`: 해당 방향 시그널이 반대 방향 시그널 전까지 `3회` 누적됨
- `+10%` 줄바꿈 `(26-04-04 09:00)`: 최초 시그널 종가 대비 현재 봉 시가가 `10%` 상승했고, 최초 시그널 종가 시간은 `26-04-04 09:00`임
