...
| 리포트 | 롱/숏 이벤트 시그널을 나열하고, 최근 기준 시그널 묶음의 누적 상태를 `ON(개수)`로 보여줍니다. 또한 `기준 시간`, `총 점수`, `진행 상태`를 함께 보여줍니다. 진행 상태는 첫 시그널이 발생한 순간 `ON`이 되고, 반대 방향 첫 시그널이 나오면 이전 묶음은 종료됩니다. | `리포트 표시`, `리포트 표시 위치`, `리포트 글자 크기`, `기준 시간 표시 시간대` |
...
`기준 시간`, `총 점수`, `진행 상태` 값은 가독성을 위해 흰색으로 고정해 표시합니다.
