...
1. [`비정상 가격 추적 (캔들)`](./비정상%20가격%20추적%20(캔들)/README.md)로 실제 자리를 확인합니다.  
   비정상 캔들, 다이버전스, ATR 배경, 진입 후보, 그 이후의 스윕 확인이 겹치는지 보고, 리포트에서 `포지션`, 기준 시간, `종가 변화`, `최대 유리`, `최대 불리`까지 함께 확인합니다. 이 지표의 모든 신호는 봉 마감 후 종가 확정 기준입니다.
...
- [`비정상 가격 추적 (캔들)`](./비정상%20가격%20추적%20(캔들)/README.md) 리포트에서 롱 반등 감시가 진행 중이고 `포지션`이 `후보` 이상이며 `최대 유리`가 커지고 `최대 불리`가 얕게 유지됩니다. 강한 강세 배경이 연속될 때 의미 있는 눌림 뒤 회복이 붙으면 더 좋지만, `ATR 강화` 보너스는 `2~4봉`까지만 커지고 이후에는 유지나 감점으로 바뀌며 강한 상승 연장 구간의 추가 롱은 차단됩니다.
...
- [`비정상 가격 추적 (캔들)`](./비정상%20가격%20추적%20(캔들)/README.md) 리포트에서 숏 반등 감시가 진행 중이고 `포지션`이 `후보` 이상이며 `최대 유리`가 커지고 `최대 불리`가 얕게 유지됩니다. 강한 약세 배경이 연속될 때 의미 있는 반등 눌림 뒤 재하락이 붙으면 더 좋지만, `ATR 강화` 보너스는 `2~4봉`까지만 커지고 이후에는 유지나 감점으로 바뀌며 강한 하락 연장 구간의 추가 숏은 차단됩니다.
...
- 리포트 `포지션`이 `관찰`인데 `최대 불리`가 계속 커질 때
