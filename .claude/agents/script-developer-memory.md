...
## 🔒 최우선 규칙 — 커밋 금지(CEO 직접 지시 2026-08-27, 항구 적용)

- **🚫 CEO가 "커밋 & 푸시"를 명시적으로 명령하기 전에는 어떤 커밋도 하지 않는다.**
  - `git commit` 금지 · `git push` 금지 · **태그 생성 금지**(`git tag`도 포함).
  - 작업 결과는 **워킹트리에만** 남긴다. 지시받지 않은 커밋은 **규칙 위반**이다.
  - 커밋하고 싶은 상황(안정 단위 도달·작업 완료 등)이 와도 **커밋하지 말고 본부장에게 보고**한다. 본부장이 CEO께 승인을 요청한다.
  - 메모리·문서(`.claude/**`·README) 수정도 예외가 아니다 — 전부 워킹트리 보류.
- **⚠️ 이 규칙은 종전 "안정 단위마다 즉시 커밋" 수칙을 대체·무효화한다.** 상충 시 **CEO 지시가 절대 우선**. 습관적 커밋 금지.
- 이 규칙의 해제는 **CEO의 명시적 지시로만** 이루어진다(본부장 전달 포함). 세션이 바뀌어도 기본값은 **커밋 금지**다.

---

## 🚨 브랜치 전략 — CEO 지시(2026-08-27, 항구 적용)

- **🚫 별도 피처 브랜치 생성 금지. 모든 작업은 `main` 브랜치에서 직접 수행한다.**
  - 신규 작업 시 `git checkout -b` **금지**. `main`에서 직접 편집(커밋은 CEO 지시 있을 때만 — 상단 최우선 규칙).
  - 이 규칙은 이후 **모든 지시에 항구 적용**된다(이전 메모리의 `refactor/…`·`feat/…` 브랜치 전략 서술은 이 항목으로 **대체·폐기**).
  - 배경: 미병합 피처 브랜치가 누적돼 검증 지연·병합 부채가 실제 발생 → CEO가 분기 실익을 인정하지 않음.
- **main 직접 작업이라 안전 수칙이 더 중요해진다(전부 유지):**
  - ~~✅ 안정 단위마다 즉시 커밋~~ → **🚫 CEO 지시로 무효화됨(2026-08-27).** 상단 "최우선 규칙 — 커밋 금지" 참조. **커밋하고 싶으면 커밋하지 말고 본부장에게 보고**한다(승인 없는 커밋 금지). 미커밋 유실 방지는 이제 '보고'로 대체한다 — 작업 종료 시 미커밋 파일 목록을 반드시 보고할 것.
  - ✅ **푸시는 CEO 승인 시에만.** main에서 작업한다고 자동 푸시하지 않는다. 병합·강제 푸시도 동일.
  - 🚫 **툴팁 문구 임의 수정 절대 금지**(CEO 관리 영역). "문서 동기화" 명목으로도 손대지 않는다 — 스테일해지면 보고만.
  - 🚫 **전략 파라미터(임계·기간) 무단 변경 금지.** 지시받은 범위 안에서만.
- **병합 이력(2026-08-27):** `feat/auto-vwap-slope-display-mode`(`2cff77d`)를 CEO 승인으로 `main`에 **fast-forward 병합 + origin 푸시** 완료(26커밋). 이후 신규 브랜치 생성 없음.
- **보존 브랜치:** `origin/feat/auto-vwap-di-gate`(원격 팁 `868664d`) — **삭제 금지**. 이번 병합 범위 제외, 처리 여부는 CEO 별도 결재 대기.
