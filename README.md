...
### 장기추세 정합 롱숏 포지션 진입/청산 라벨

위 매수/매도 삼각형과 별도로, **장기추세와 정합**되는 방향으로만 진입하고 **샹들리에 청산선 이탈** 시 청산하는 **실매매 규칙**을 차트에 **라벨(`label`)** 로 표시합니다.

| 포지션 | 진입 (**flat일 때만**) | 청산 (**보유 중**) |
| --- | --- | --- |
| **롱** | 캔들 매수색(교차검증 정렬) + **장기추세 상승** + 종가가 **청산선 사이**(손익비 성립) | 종가가 **롱 청산선 하향 이탈**(`close < longClear`) |
| **숏** | 캔들 매도색(교차검증 정렬) + **장기추세 하락** + 종가가 **청산선 사이** | 종가가 **숏 청산선 상향 이탈**(`close > shortClear`) |

- **상태머신 · 중복 진입 금지**: `posState`(flat/롱/숏)로 관리 — flat일 때만 진입(롱/숏 배타), 보유 중엔 해당 청산만, **청산 완료 후에만** 재진입(같은 봉 청산+즉시 재진입 금지).
- **확정봉 기준(리페인팅 없음)**: 상태 전이·라벨 생성을 모두 `barstate.isconfirmed`에서만. 진입=봉 위(녹/빨 선명), 청산=봉 아래(옅은 톤). 표시 토글 `롱숏 포지션 라벨 표시`(기본 on).
- **★ plot count 무소비**: `label.new`로만 구현(plot/plotshape 신규 추가 0) + `max_labels_count=500`(최근 500 이벤트 라벨 유지). 이전 `plotshape` 구현이 plot count 상한 초과로 지표 미표시 크래시를 낸 전례를 label 방식으로 원천 차단했습니다.

⚠️ 실매매 규칙이나 **백테스트(엣지 검증)는 미실시** — 알고리즘 수학자 후속 검증 권고. 상세는 [`pinescript/SUPER-ICHIMOKU/README.md`](pinescript/SUPER-ICHIMOKU/README.md) 참조.
...
| 행 | 값 | 셀 색 |
| --- | --- | --- |
| `캔들 상태 (3)` | 최근 **3봉을 7분류**로 스코어링해 `분류명 \| 근거패턴`. **표시 전용**(신호 미연결). 현재봉 캔들 바디도 이 색으로 칠함(혼조 제외) — **[캔들 색 표시 조건] 3조건 AND 게이트**(추세 정렬·리본 국면·되돌림 구간, **TF별 2세트 자동 선택**) 모두 만족 시에만 색 표시(셀 색은 게이트와 무관하게 항상 유지) | 7분류 색(아래) |
...
> **캔들 색 표시 조건(AND 게이트 · TF별 2세트 자동 선택)** — 현재봉 캔들 바디 색칠(`barcolor`)은 **[캔들 색 표시 조건]** 3조건(① 추세 정렬 최소 신호 수 · ② 리본 국면 조건 · ③ 되돌림 구간 처리)을 **모두 만족(AND)** 할 때만 노출됩니다. 세 조건은 **1D 미만(인트라데이)**·**1D 이상(스윙)** 두 세트로 나뉘어 차트 TF(`timeframe.in_seconds()<86400`)에 따라 자동 선택됩니다. 리본 국면 조건은 **강도 순 4옵션**(`이상`=그 강도부터 더 정합한 쪽 모두 허용): `포지션 방향 분출 이상`(정합 분출만) → `횡보 압축 이상`(+횡보) → `반대 포지션 방향 약분출 이상`(+반대 분출이라도 리본 이동% 절대값이 `반대 분출 허용 임계%(약분출)` 이하면 허용, 세트별 기본 1D 미만 1.0% / 1D 이상 2.0%) → `무관`. 기본값: **1D 미만** = 추세 정렬 `2`·리본 `반대 포지션 방향 약분출 이상`(허용 임계 1.0%)·되돌림 `되돌림 확실 시 색 숨김`(엄격), **1D 이상** = 추세 정렬 `1`·리본 `반대 포지션 방향 약분출 이상`(허용 임계 2.0%)·되돌림 `무관`(완화). 교차검증 정렬 시에만 캔들 색을 보여 신뢰도를 높입니다. **표시 전용** — 캔들 7분류·바색 색값·테이블 셀 색은 무변경(셀 색은 항상 유지). 자세한 정의는 [SUPER-ICHIMOKU README](pinescript/SUPER-ICHIMOKU/README.md#캔들-색-표시-조건--교차검증-정렬-and-게이트표시-전용) 참조.
...
> 이 정렬 확인 진입/청산은 **[장기추세 정합 롱숏 라벨](#장기추세-정합-롱숏-포지션-진입청산-라벨)** 으로 차트에 자동 표시됩니다(캔들 매수/매도색 + 장기추세 정합 + 청산선 사이 진입, 청산선 이탈 시 청산, 상태머신 중복 금지·확정봉 기준). 라벨은 규칙의 **시각적 보조**이며, 진입은 반드시 위 표(VWAP·보조지표·손익비)와 교차확인 후 CEO 승인으로 실행합니다.
