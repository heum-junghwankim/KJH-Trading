...
- 차트 **우측 하단 트리거 테이블**: 다음 봉 종가가 어느 가격을 돌파해야 `상승/하락 후보`(래깅 vs 컨버전 라인)·`상승/하락 신호`(래깅·컨버전 vs 기준선)가 나오는지 미리 표시(중복 방지로 막힌 방향은 `-`). 같은 표에 현재 봉의 `구름대`(위치·두께·색)와 `이평 리본`(압축/분출, 차트 다이아몬드와 동기화) 상태도 괄호 숫자값(두께/폭 ÷ ATR)과 함께 표시.
- 캔들 아래 **리본 압축/분출 다이아몬드**(이치모쿠 인디케이터·전략에 내장): 20~50 이평이 뭉치면 `횡보 압축`(회색), 풀리면 `상승 분출`(파랑)/`하락 분출`(빨강) → 휩쏘 필터.
