...
#### 1-5. 롱숏 포지션 진입/청산 라벨 (실매매 규칙)

`Cha` 손익비 게이트를 통과한 자리에서 **차트 위에 진입/청산 라벨**을 자동으로 찍습니다. 테이블이 아니라 봉 위/아래 라벨(`label`)로 표시되며, plot count를 소비하지 않습니다.

- **진입 게이트**: 방향 신호(매수/매도 확정 발화 `sBuy10`/`sSell10`, 옵션으로 0.5× 포함) + **장기 추세 정렬**(롱=우상향·숏=우하향) + 청산선 유효(`chaValid`) + **`Cha` RR ≥ 하한(`rrMin`, 기본 1.5)**.
- **손절**: 청산선(롱=롱 청산선·숏=숏 청산선). **익절**: `max/min(반대편 청산선, 진입가 ± R배수 × 위험폭)`(`R배수` 기본 2.0).
- **상태머신**: 롱/숏 **완전 독립**(동시 보유 가능), 같은 방향은 flat일 때만 진입(중복 금지). 진입가·손절·목표는 진입 시점에 고정(래치)해 리페인팅 없음.
- **청산 사유**: 손절 도달 / 익절 도달 / 추세 이탈(반대 신호 or 장기 추세 반전). 라벨에 사유 + 수익률%를 표기(진입 라벨엔 손절%).
- 파라미터(`rrMin`·`R배수`·0.5× 허용)는 **잠정치**이며 **엣지 미검증 — 알고리즘 수학자의 백테스트로 확정**합니다.
...
> 이 정렬 진입은 **롱숏 포지션 라벨**(1-5)로 차트에 자동 표기됩니다 — `Cha` RR 하한을 통과하고 장기 추세가 정렬됐을 때만 진입 라벨을 찍고, 손절(청산선)·익절(2R/반대편 청산선)·추세 이탈 시 청산 라벨로 사유+수익률%를 남깁니다. 규칙·파라미터는 잠정(백테스트 확정 전).
