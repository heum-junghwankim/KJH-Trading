...
#### 1-5. 롱숏 포지션 진입/청산 라벨 (실매매 규칙 · 4근거 중 3↑ · 실시간)

**차트 위에 진입/청산 라벨**을 자동으로 찍습니다. 테이블이 아니라 봉 위/아래 라벨(`label`)로 표시되며, plot count를 소비하지 않습니다.

> **[CEO 지시]** 진입은 **4개 방향 근거 중 3개 이상이 같은 방향**일 때만 성립합니다. 단일 신호를 불신하고 교차 검증으로 선별하되(라벨 과다 방지), **실시간**으로 표시합니다(리페인팅 감수).

- **진입 방향(4근거 중 3↑)**: 아래 4개 근거 중 **3개 이상**이 같은 방향이면 진입 — ① **리본 방향**(상승/하락 분출 `ribbonState`) · ② **추세 신호**(발화 `sBuy/sSell` 0.5×/1×/2× **or** `dir` 추세 상태) · ③ **비정상 방향 주문량**(이치모쿠 내 비정상 거래량 `abnVolume` + 그 봉 방향 = 롱은 양봉 `close>open`·숏은 음봉 `close<open`) · ④ **캔들 상태 방향**(`candleIsUp`/`candleIsDown`).
- **필수조건 없음**: **RR 게이트** · **장기추세 정렬(`ltUp`)** · **청산선 유효(`chaValid`)** 모두 진입 조건 아님. 3/4 근거 자체가 진입 기준이며, 남는 조건은 **그 방향 flat**(중복 금지)뿐.
- **손절**: 청산선(방향 정합 시 — 롱은 진입가 아래, 숏은 위), 청산선이 진입가 반대편이면 **ATR(14) 폴백**(진입가 ∓ ATR×배수, 방향 정합 보장). **익절**: `max/min(반대편 청산선, 진입가 ± R배수 × 위험폭)`(`R배수` 기본 2.0, 가성비 RR). 위험폭 = |진입가 − 손절|.
- **롱↔숏 방향 전환**: 롱/숏 **독립 상태머신**(동시 보유 가능), 같은 방향은 flat일 때만 진입(중복 금지). 청산 즉시 flat 복귀 → **롱 청산과 무관하게 숏 조건이 충족되면 숏 진입**(방향 전환 미차단). 진입가·손절·목표는 진입 시점에 고정(래치).
- **청산 사유**: 손절 도달 / 익절 도달. **진입 봉 동일봉 청산은 금지**(진입가=close인데 그 봉의 지나간 low/high로 판정하는 룩백 왜곡 차단 — 다음 봉부터 검사). 추세 이탈은 반대 방향 독립 진입이 대체(청산 즉시 flat). 라벨에 사유 + 수익률%를 표기(진입 라벨엔 손절%). **실시간 판정**이라 봉 완성 전 라벨 변동(리페인팅)은 감수(룩어헤드 없음).
- 파라미터(`R배수`·`폴백 손절 ATR 배수`)는 **잠정치**이며 **엣지 미검증 — 알고리즘 수학자의 백테스트로 확정**합니다. 리페인팅은 의도된 동작입니다.
...
> 이 정렬 진입은 **롱숏 포지션 라벨**(1-5)로 차트에 자동 표기됩니다 — **[4근거 중 3↑·실시간]** RR 게이트·장기추세 정렬 없이 **4개 방향 근거(리본·추세·비정상 방향 주문량·캔들 상태) 중 3개 이상이 같은 방향**이면 진입 라벨을 찍고, 손절(청산선 정합, 아니면 ATR 폴백)·익절(2R/반대편 청산선) 도달 시 청산 라벨로 사유+수익률%를 남깁니다. 롱/숏 독립이라 롱 청산 후 숏 진입(방향 전환) 허용. 실시간이라 리페인팅 감수. 규칙·파라미터는 잠정(백테스트 확정 전).
