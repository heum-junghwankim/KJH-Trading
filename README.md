...
| 행 | 내용 |
| --- | --- |
| `상승/하락 전환` | 다음 봉 종가가 어느 가격을 돌파해야 매수/매도 신호가 되는지 `↑/↓ 가격 (현재가 대비 %)`. **0.5× / 1× / 2× 세 스케일** 각각. 트리거 근접(±2% 이내)·충족 시 포지션 색으로 강조. 이미 그 방향이 진행 중(중복 막힘)이면 `상승 추세`/`하락 추세` 표시 |
| `구름대` | **지지/저항 강도**: 지그재그 레그가 구름 안이면 하락 중=`지지 강함/보통/약함`·상승 중=`저항 강함/보통/약함`(구름 두껍고 접근 각도 완만할수록 강함), 밖이면 위/안/아래·양운/음운 |
| `중기 추세` | (구 이평 리본) `횡보 압축 / 상승 분출 / 하락 분출` + 이벤트 후 가격 변화 %, 차트 다이아몬드와 동기화 |
| `장기 추세` | MA100·200 기반 `강한/약한 상승·하락 추세`, `골든/데드 크로스`(약한 추세엔 크로스 주의) |
| `롱/숏 스윕·청산` | 샹들리에 `스윕`(점선, 스톱헌트 자리)·그 너머 `청산`(직선) 가격. 트레일+수평선(미래 20봉·1px, 롱=파랑·숏=크림슨) |
| `롱/숏 손익비` | 현재가 기준 청산가 손익비(목표÷손절) `비율 (+목표%/-손절%)`. 현재가가 청산가 밖이면 `계산 불가` |
| `VWAP` 3행 | Auto VWAP 통합(**계산 전용, 차트 미표시**) — `위치` / `모양`(기울기\|밴드폭) / `밴드 상태`(밴드워킹 vs 지지·저항) |
| `비정상 캔들` | cRSI 과열+볼린저 이탈 **비정상 캔들**(현재 봉 표시) 후 ATR 추종 배경 `롱/숏 감시(강)`·`비정상 매도/매수 캔들`(이격도 중립까지 유지·최신 우선), **이격도가 중립으로 복귀하면 `정상 캔들`**. 차트엔 바색+배경색. **롱 반등=매수 신호만, 숏 반등=매도 신호만 더 빠른 로직(래깅 위치 봉 종가 돌파)으로 전환**(트리거도 동기화) |
