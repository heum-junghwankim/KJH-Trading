...
| 행 | 내용 |
| --- | --- |
| `상승/하락 전환` | 다음 봉 종가가 어느 가격을 돌파해야 매수/매도 신호가 되는지(`↑/↓ 가격`). **0.5× / 1× / 2× 세 스케일을 각각** 표시. 중복 방지로 막힌 방향은 `-` |
...
> **리본 압축/분출 다이아몬드**(이치모쿠 인디케이터에 내장)의 `횡보 압축`(캔들 아래 회색) 이후 리본이 다시 벌어지기 전까지의 구간은 20~50 이동평균들이 뭉쳐 방향이 사라진 휩쏘 구간이라, 위 정렬이 일치해도 신뢰를 한 단계 낮춰 보는 게 안전합니다. 압축이 풀리는 순간 방향대로 `상승 분출`(파랑)/`하락 분출`(빨강)이 찍히는데, 이치모쿠 신호 방향과 같으면 신뢰가 큽니다. 진입 트리거가 아니라 신뢰도 필터로만 씁니다.
