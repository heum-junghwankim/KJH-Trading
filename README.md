...
#### 1-5. 롱숏 포지션 진입/청산 라벨 (실매매 규칙 · 허들 완화 · 실시간)

**차트 위에 진입/청산 라벨**을 자동으로 찍습니다. 테이블이 아니라 봉 위/아래 라벨(`label`)로 표시되며, plot count를 소비하지 않습니다.

> **[CEO 지시]** 기존 게이트가 너무 엄격해(백테스트에서 라벨이 거의 안 뜸) **진입 허들을 대폭 낮췄습니다.** 방향 근거 중 **하나라도** 뜨면 진입하고(느슨한 OR), **실시간**으로 표시합니다(리페인팅 감수).

- **진입 방향(느슨한 OR)**: 아래 근거 중 **하나 이상**이면 진입 — ① **추세**(발화 신호 `sBuy/sSell` 0.5×/1×/2× **or** `dir` 추세 상태) · ② **캔들 상태**(상승/하락 계열) · ③ **VWAP 밴드 수축 + 기울기 방향**. 각각 input 토글로 켜고 끕니다.
- **제거된 필수조건**: **RR 게이트**(진입 차단 안 함 — RR은 라벨에 표시만) · **장기추세 정렬(`ltUp`)** · **청산선 유효(`chaValid`)**. 남는 조건은 **방향 OR + 그 방향 flat**(중복 금지)뿐.
- **손절**: 1순위 청산선(방향 정합 시), 청산선이 진입가 반대편이면 **폴백** — 캔들 3봉 극점(정합 시)과 **ATR 손절**(진입가 ∓ ATR×배수) 중 더 보수적인 쪽(방향 정합 보장). **익절**: `max/min(반대편 청산선, 진입가 ± R배수 × 위험폭)`(`R배수` 기본 2.0).
- **상태머신**: 롱/숏 **완전 독립**(동시 보유 가능), 같은 방향은 flat일 때만 진입(중복 금지). 진입가·손절·목표는 진입 시점에 고정(래치). **실시간 판정**이라 봉 완성 전 라벨 변동(리페인팅)은 감수(룩어헤드 없음).
- **청산 사유**: 손절 도달 / 익절 도달 / **반대신호**(반대편 느슨한 방향, 진입 봉 당일은 제외). 라벨에 사유 + 수익률%를 표기(진입 라벨엔 손절% + RR).
- 파라미터(`R배수`·`폴백 손절 ATR 배수`·근거 토글)는 **잠정치**이며 **엣지 미검증 — 알고리즘 수학자의 백테스트로 확정**합니다. 리페인팅은 의도된 동작입니다.
...
> 이 정렬 진입은 **롱숏 포지션 라벨**(1-5)로 차트에 자동 표기됩니다 — **[허들 완화·실시간]** 이제 RR 게이트·장기추세 정렬 없이 **방향 근거(추세·캔들·VWAP 수축) 중 하나만** 떠도 진입 라벨을 찍고, 손절(청산선 정합, 아니면 ATR/캔들 폴백)·익절(2R/반대편 청산선)·반대신호 시 청산 라벨로 사유+수익률%를 남깁니다. 실시간이라 리페인팅 감수. 규칙·파라미터는 잠정(백테스트 확정 전).
