...
| 신호 표시 위치 | 매수/매도 삼각형 위치를 각각 4옵션(**봉 위 / 봉 아래 / 화면 상단 / 화면 하단**)에서 선택. **기본 매수=화면 하단·매도=화면 상단**(가격 무관·화면 절대라 캔들·구름과 안 겹쳐 신호 열이 한눈에). 위치만 바뀌고 신호 판정·색·모양은 무변경 |
...
| 롱숏 포지션 플래그(상태머신) | 위 삼각형과 별개로, **장기추세 정합 진입/청산 규칙**을 봉 위/아래 🏳 플래그로 표시. 진입=캔들 매수/매도색 게이트+장기추세 정합+종가가 롱·숏 청산선 사이, 청산=확정봉 종가가 해당 청산선 이탈. **상태머신으로 중복 진입 차단**(무포지션일 때만 진입·청산 후 재진입). **확정봉에서만 전이**(리페인팅 없음). ⚠️ 엣지 미검증(백테스트 전) |
...
> **롱숏 포지션 플래그(보조 참고)**: 이 정렬 진입을 코드가 자동 판정해 봉 위/아래 🏳 플래그로 표시합니다 — 진입=캔들 매수/매도색 게이트 통과 + 장기추세 정합 + 종가가 롱·숏 청산선 사이, 청산=확정봉 종가가 해당 청산선 이탈. **상태머신으로 중복 진입을 막고, 청산 후에만 재진입**합니다. 리페인팅 방지를 위해 **확정봉에서만** 상태가 전이됩니다(진입 플래그는 봉 완성 시 확정 표시). ⚠️ **엣지 미검증** — 백테스트 전이므로 단독 매매 근거가 아니라 정렬 확인 보조로만 씁니다.
