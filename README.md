...
> **캔들 색 표시 조건(AND 게이트 · TF별 2세트 자동 선택)** — 현재봉 캔들 바디 색칠(`barcolor`)은 **[캔들 색 표시 조건]** 3조건(① 추세 정렬 최소 신호 수 · ② 리본 국면 조건 · ③ 되돌림 구간 처리)을 **모두 만족(AND)** 할 때만 노출됩니다. 세 조건은 **1D 미만(인트라데이)**·**1D 이상(스윙)** 두 세트로 나뉘어 차트 TF(`timeframe.in_seconds()<86400`)에 따라 자동 선택됩니다. 기본값: **1D 미만** = 추세 정렬 `2`·리본 `추세 분출 또는 횡보 수축`·되돌림 `되돌림 확실 시 색 숨김`(엄격), **1D 이상** = 추세 정렬 `1`·리본 `무관`·되돌림 `무관`(완화). 교차검증 정렬 시에만 캔들 색을 보여 신뢰도를 높입니다. **표시 전용** — 캔들 7분류·바색 색값·테이블 셀 색은 무변경(셀 색은 항상 유지). 자세한 정의는 [SUPER-ICHIMOKU README](pinescript/SUPER-ICHIMOKU/README.md#캔들-색-표시-조건--교차검증-정렬-and-게이트표시-전용) 참조.
