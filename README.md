...
| 행 | 내용 |
| --- | --- |
| `상승/하락 전환` | 다음 봉 종가가 어느 가격을 돌파해야 매수/매도 신호가 되는지 `↑/↓ 가격 (현재가 대비 %)`. **0.5× / 1× / 2× 세 스케일** 각각. 트리거 근접(±2% 이내)·충족 시 포지션 색으로 강조. 이미 그 방향이 진행 중(중복 막힘)이면 `상승 추세`/`하락 추세`를 구름대 셀과 같은 채도의 포지션 색으로 표시 |
...
| `비정상 캔들` | cRSI 과열+볼린저 이탈 **비정상 캔들** 후 ATR 추종 배경 `롱/숏 감시(강)`·`비정상 매도/매수 캔들`, 밴드워킹 종료(밴드 선 기울기 반전) 시 `정상 캔들`. 차트엔 비정상 캔들 바색 + 배경색 |
| `롱/숏 손익비` | 현재가 기준 청산가 손익비(목표÷손절). 현재가가 청산가 밖이면 `계산 불가` |
| `VWAP` 4행 | Auto VWAP 통합(**계산 전용, 차트 미표시**) — `위치/기울기/밴드폭/밴드 상태`(밴드워킹 vs 지지·저항) |
