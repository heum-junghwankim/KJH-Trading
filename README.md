...
- 같은 방향 신호는 반대 신호가 한 번 나온 뒤에만 다시 발생(중복 방지). 신호보다 빠른 **후보 동그라미**가 먼저, 표준 신호와 함께 **거래량 가중(VWMA) 반투명 마커**도 동시 표시.
- ⚠️ 추세추종이라 **횡보장 휩쏘 / 후행성** 약점 → 그대로 믿지 말고 아래 상태판·리본 필터로 검증.

차트 **우측 하단 테이블**(인디케이터·전략 공통):

| 행 | 내용 |
| --- | --- |
| `상승/하락 전환` | 다음 봉 종가가 어느 가격을 돌파해야 매수/매도가 되는지(후보·신호 트리거 결합, `↑/↓ 가격`). 중복 방지로 막힌 방향은 `-` |
| `구름대` | 현재 봉 가격 위치·양운/음운·두께(÷ATR) |
| `이평 리본` | 20~50 리본 `횡보 압축 / 상승 분출 / 하락 분출` 상태(÷ATR), 차트 다이아몬드와 동기화 |
| `롱/숏 청산` | 샹들리에 ATR 트레일링 청산가 |

- 캔들 위·아래 **리본 압축/분출 다이아몬드**(휩쏘 필터): 20~50 이평이 뭉치면 `횡보 압축`(회색), 풀리면 `상승 분출`(파랑)/`하락 분출`(빨강).
...
> **리본 압축/분출 다이아몬드**(이치모쿠 인디케이터·전략에 내장)의 `횡보 압축`(캔들 아래 회색) 이후 리본이 다시 벌어지기 전까지의 구간은 20~50 이동평균들이 뭉쳐 방향이 사라진 휩쏘 구간이라, 위 정렬이 일치해도 신뢰를 한 단계 낮춰 보는 게 안전합니다. 압축이 풀리는 순간 방향대로 `상승 분출`(파랑)/`하락 분출`(빨강)이 찍히는데, 이치모쿠 신호 방향과 같으면 신뢰가 큽니다. 진입 트리거가 아니라 신뢰도 필터로만 씁니다.
