...
| **`혼합 일반 모드`**(기본) | **봉별 스위칭** — 비정상 거래량 봉=VWMA **즉시 발화**, 정상 봉=일반 도치안 | 정상 봉만 예측, 비정상 봉은 즉시 |
...
#### 1-5. 롱숏(L/S) 포지션 진입/청산 라벨 (실매매 규칙 · VWAP 상태·추세신호 개수 기반 · 실시간)

**차트 위에 진입/청산 라벨**을 자동으로 찍습니다. 테이블이 아니라 봉 위/아래 라벨(`label`)로 표시되며, plot count를 소비하지 않습니다.

> **[CEO 규칙 재정의]** 진입은 **VWAP 상태 + 추세신호 개수**(A) 또는 **비정상 방향 주문량 + 추세신호 개수**(B) 중 하나로 성립하고, 청산은 **VWAP 방향**에 따라 반대신호/손절선 이탈 또는 트레일링 스톱 익절로 갈립니다. **실시간**으로 표시합니다(리페인팅 감수).

- **추세신호 개수(0~3)**: 스케일별 1개씩(0.5×/1×/2×) — 각 스케일에서 발화(`sBuy/sSell`) **OR** `dir` 추세(`dir05/10/20`==방향)이면 1.
- **진입 = (A) OR (B)** — **롱**: (A) `(VWAP 평탄 OR VWAP 하락[롱 반대]) AND 추세신호≥2` · (B) `(비정상 상승 주문량) AND 추세신호≥1`. **숏 대칭**: (A) `(평탄 OR 상승) AND 추세≥2` · (B) `비정상 하락 AND 추세≥1`. 비정상 방향 주문량 = `abnVolume` + 그 봉 방향(롱 양봉·숏 음봉). VWAP 상태 = `vwSlopeFlat`(평탄)·`vwSlopeUp`(상승)·`vwSlopeDown`(하락).
- **⚠️ VWAP 같은방향 = (B) 전용**: VWAP이 포지션과 **같은 방향**(롱↑·숏↓)이면 (A)의 VWAP 조건이 미충족 → **(B)로만 진입 가능**한 구조(CEO 규칙 문자 그대로 — 의도와 다르면 조정).
- **손절(진입 시 확정·래치)**: **청산선 밴드 안**(`chaValid`, 종가가 롱·숏 청산선 사이)이면 방향 청산선(롱 `longClear`·숏 `shortClear`), **밴드 밖**이면 **진입 캔들 손절라인**(롱=3봉 최저 `candleStopLow3`·숏=3봉 최고 `candleStopHigh3`).
- **청산** — **VWAP 평탄/반대**: 반대 포지션 추세신호 ≥2 → 청산(반대신호), 또는 손절선 이탈 → 손절. **VWAP 같은방향**: 청산선이 트레일링 스톱이 되어 종가가 청산선을 역크로스(롱=`longClear` 하향·숏=`shortClear` 상향)하면 → **익절**.
- **롱↔숏 방향 전환**: 롱/숏 **독립 상태머신**(동시 보유 가능), 같은 방향은 flat일 때만 진입(중복 금지). 청산 즉시 flat → **롱 청산 후 숏 진입 허용**(방향 전환 미차단). 진입가·손절은 진입 시점에 var 래치. **진입 봉 동일봉 청산 금지**(룩백 왜곡 차단).
- **라벨(L/S 명시)**: 롱 `L 포지션 진입 (−N%)`·숏 `S 포지션 진입 (+N%)`(N%=손절 비율), 청산 `L/S 포지션 청산 (사유 ±N%)`(사유=반대신호/손절/익절, 손익%). **실시간 판정**이라 봉 완성 전 라벨 변동(리페인팅)은 감수(룩어헤드 없음).
- 진입·청산 규칙은 **엣지 미검증 — 알고리즘 수학자의 백테스트로 확정**합니다. 리페인팅은 의도된 동작입니다.
...
> 이 정렬 진입은 **롱숏(L/S) 포지션 라벨**(1-5)로 차트에 자동 표기됩니다 — **[VWAP 상태·추세신호 개수·실시간]** 진입은 **(A) `(VWAP 평탄/반대) AND 추세신호≥2`** 또는 **(B) `비정상 방향 주문량 AND 추세신호≥1`**(VWAP 같은방향이면 (B) 전용). 손절은 밴드 안이면 청산선·밖이면 캔들 손절라인(3봉 극점), 청산은 VWAP 평탄/반대에서 반대신호 2개/손절선 이탈, VWAP 같은방향에서 청산선 트레일 크로스 익절로 사유+손익%를 남깁니다. 롱/숏 독립이라 롱 청산 후 숏 진입(방향 전환) 허용. 실시간이라 리페인팅 감수. 규칙은 잠정(백테스트 확정 전).
