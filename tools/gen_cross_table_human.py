...
프로그램/자동조회용은 별도 10축 대용량 CSV(gen_cross_table_program.py)를 참조(로직 무변경).
...
> 조합 행의 **방향·신뢰도·해석**을 3초 안에 찾는 용도. 자동조회용 10축 완전 열거는
> 별도 CSV `pinescript/신호-교차표-프로그램용.csv` (아래 "프로그램용 CSV 안내" 참조).
>
...
- **무엇인가:** 이치모쿠 **10축**을 각 3상태로 정규화해 **완전 열거(3^10 = 59,049조합)**한 표를 담은 **CSV(표만·산문 없음)**. 사람이 훑는 용도가 아니라, 실차트 이치모쿠 테이블의 10개 상태값을 그대로 읽어 같은 조합 행을 **자동/프로그램으로 조회**하는 용도. 생성기 `tools/gen_cross_table_program.py`(결정론적·재실행 시 동일).
- **컬럼:** `No · 추세 · 리본추세 · 이격도 · 구름대 · VWAP기울기 · VWAP밴드폭 · VWAP밴드상태 · VWAP지지저항 · 장기추세 · 장기지지저항 · 방향 · 신뢰도 · 종합 해석`.
- **사람용(6축) vs 프로그램용(10축) 차이:** 사람용은 눈으로 3초에 읽는 6축을 신뢰도 내림차순 슬림표(1,458행)로 정리한 것. 프로그램용은 VWAP를 **기울기·밴드폭·밴드상태·지지저항 4축으로 분해**하고 **장기지지저항 축을 추가**한 10축을, No 순차(축 사전순)로 완전 열거(59,049행)한 것 — 방향 판정 로직은 동일 계열이나 축 구성이 더 세밀하다.
...
- **장기추세 (8→3):** `골든 크로스·강한 상승·상승·약한 상승`→**상승** / `데드 크로스·강한 하락·하락·약한 하락`→**하락** / 확립 전→**중립**. (강/약은 신뢰도 세분, 방향은 `ltUp` 부호로 상승/하락 통합.)
- **장기지지저항 (100선×200선 9→3):** 장기 기준인 200선 우선. `200선 지지` 계열→**지지우세** / `200선 저항` 계열→**저항우세** / `200선 중립`→**혼조**. (`ltM2St` 부호 = 셀 배경색 기준이라 200선을 대표로 축약.)
- **VWAP밴드상태 (4→3):** `상단·하단 밴드워킹`→**밴드워킹** / `하단 지지`→**지지** / `상단 저항`→**저항**. `중앙(공방)`은 별도 상태 없이 방향 판정에서 **중립(투표 0·보정 0)**으로 보고 종가가 가까운 밴드 쪽 행 참조(이 축은 신뢰도 보정축이라 방향 결론 영향 작음).
- **리본추세:** `대기`(초기)→**횡보**. **VWAP기울기 `평탄(중심)`**→**평탄**(방향 0).

### 프로그램용 방향 판정 규칙

- **투표축 6개** (롱 +1 / 숏 -1 / 중립 0): 추세·구름대·장기추세·장기지지저항·VWAP지지저항·리본. 부호합 `vsum`(-6..+6).
- **롱 정렬** = `vsum ≥ +3` AND 반대표 0개. **숏 정렬** = `vsum ≤ -3` AND 반대표 0개. 그 외 = 중립·혼조.
- **신뢰도** = `|vsum|/6` × 이격도페널티(중립1.00·주의0.85·확실0.70) + 보정축(정렬 행에만): VWAP기울기 정렬방향 일치 +0.05·반대 -0.10·평탄 0 / VWAP밴드폭 확장 +0.05·수축 -0.05·유지 0 / VWAP밴드상태 밴드워킹 +0.05·지지·저항 0. `clamp(0,1)`. 등급: ≥0.80 강한 정렬 / 0.50~0.80 우세 / <0.50 약세.
  - ※ 사람용(6축)은 VWAP를 기울기 한 축으로만 투표(투표축 5개·`|vsum|/5`)해 이중카운트를 피하지만, 프로그램용은 VWAP지지저항을 별도 투표축으로 두어 6축(`|vsum|/6`)으로 계산한다 — 이 차이가 두 표의 신뢰도 스케일 차이를 만든다.
